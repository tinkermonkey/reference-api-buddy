        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            config = {
                ...
                "throttling": {
                    ...
                    "domain_limits": {
                        "httpbin": 1  # Only 1 request per hour
                    }
                },
                "security": {
            ...
            with patch('reference_api_buddy.core.handler.RequestProcessingMixin._forward_request', side_effect=mock_forward_request):
                try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('reference_api_buddy.core.handler.RequestProcessingMixin._forward_request', side_effect=mock_forward_request):
                try:
                    ...
                    throttle_events = [
                        event for event in metrics.get("events", [])
                        if event.get("event_type") == "throttle"
                    ]
